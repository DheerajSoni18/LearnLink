...
if(isset($_POST['submit']))
  {
    ...
    // $uploadName = $_POST['photo'];
    // // echo "<script>console.log(' . json_encode($uploadName) . ');</script>";
    // $fileExtension = strtolower(pathinfo($uploadName, PATHINFO_EXTENSION));
    // $allowedExtension = ['jpg', 'jpeg', 'png'];
    // $validFile = in_array($fileExtension, $allowedExtension) ? 'true' : 'false';
    // // echo "<script>console.log(' . json_encode($fileExtension) . ');</script>";
    // // echo "<script>console.log(' . json_encode($validFile) . ');</script>";

    // $targetDir = 'img/';
    // $uniqueName = time() . "_" . $uploadName;
    // $targetFilePath = $targetDir . $uniqueName;
    // // echo "<script>console.log(' . json_encode($targetFilePath) . ');</script>";
...
if($query -> rowCount() == 0)
{
    // if($validFile && move_uploaded_file($_FILES["profile_image"]["tmp_name"], $targetFilePath)) {
        ...
        // $query->bindParam(':photoName',$uniqueName,PDO::PARAM_STR);
        $query->execute();
    ...
    // }
    // else{
    //     echo "<script>alert('Failed to upload image (jpg/jpeg/png');</script>";
    // }
}
else
{
                    ...
                    <div class="bg-light rounded p-4 p-sm-5 my-4 mx-3">
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            <a href="../index.php" class="">
                                <h3 class="text-primary">LearnLink</h3>
                            </a>
                            <h3>Sign Up</h3>
                        </div>
